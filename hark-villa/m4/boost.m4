...
AC_ARG_WITH([boost],
   [AS_HELP_STRING([--with-boost=DIR],
                   [prefix of Boost $1 @<:@guess@:>@])],
                   [LDFLAGS="-L${withval}/lib ${LDFLAGS}"])dnl
...
# ACTION-IF-NOT-FOUND action if one was supplied.
# Otherwise aborts with an error message.
AC_DEFUN_ONCE([BOOST_FILESYSTEM_JUDGE],
[AC_REQUIRE([AC_PROG_CXX])dnl
AC_REQUIRE([AC_PROG_GREP])dnl
...
# ACTION-IF-NOT-FOUND action if one was supplied.
# Otherwise aborts with an error message.
AC_DEFUN_ONCE([BOOST_THREAD_JUDGE],
[AC_REQUIRE([AC_PROG_CXX])dnl
AC_REQUIRE([AC_PROG_GREP])dnl
echo "$as_me: this is boostapd.m4[]_BOOST_SERIAL" >&AS_MESSAGE_LOG_FD
boost_save_IFS=$IFS
...
AC_LANG_POP([C++])dnl
    ...
    case $boost_cv_inc_path in #(
      no)
        ...
        AC_SUBST([BOOST_THREAD_JUDGE_LIB], ["$BOOST_THREAD_JUDGE_LIB '-lboost_thread-mt'"])dnl
        ;;#(
      *)
        AC_SUBST([BOOST_THREAD_JUDGE_LIB], ["$BOOST_THREAD_JUDGE_LIB '-lboost_thread'"])dnl
        ;;
    esac
])# BOOST_THREAD_JUDGE
