...
parser.add_argument('-r', '--playrec', type=int, nargs=1, help='Only generate and record TSP')
...
# Only playrec flag
if args.playrec!=None and args.playrec[0] not in (0,1):
    print "Invalid argument for playrec"
    sys.exit()
elif args.playrec!=None:
    flg_playrec = args.playrec[0]
else:
    flg_playrec = 0
...
if dev_id<0: #default audio device
    ...
    in_dev_info = pa.get_default_input_device_info()
    out_dev_info = pa.get_device_info_by_index(dev_id)
...
tsp_data_sync = np.zeros((out_channel,tsp_len*(nsync+1)), dtype=np.int16)
for i in np.arange(nsync):
    tsp_data_sync[out_channel-1][i*tsp_len:(i+1)*tsp_len] = tsp_data

tsp_out = (tsp_data_sync.T).reshape((tsp_len*(nsync+1)*out_channel,1))
...
for i in np.arange(nframe):
    pa_input = in_stream.read(chunk)
    pa_indata.append(pa_input)
    pa_outdata = tsp_out[i*out_channel*chunk:(i+1)*out_channel*chunk]
    pa_output = out_stream.write(pa_outdata.tostring())
...
if flg_dump:
    ...
    wf.setparams((n_in_channel, 2, Fs, tsp_len*(nsync+1), 'NONE', 'not compressed'))
    wf.writeframesraw(b''.join(tsp_rcv_raw))
    wf.close()

# Quit if user only desires TSP gen/record
if flg_playrec:
    sys.exit()
...
for i in np.arange(n_in_channel):
    ...
    for j in np.arange(tsp_len*(nsync+1)):
        amp_av = amp_av + (tsp_rcv_data[j*n_in_channel+i].astype(np.float64))**2/(tsp_len*(nsync+1))
        tsp_rcv_sig[i][j] = tsp_rcv_data[j*n_in_channel+i].astype(np.float64)/float(max_amp)
    if amp_av<1.0:
